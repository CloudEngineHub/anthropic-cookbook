...
| [`CMA_plan_big_execute_small.ipynb`](CMA_plan_big_execute_small.ipynb) | The cost case for multi-agent: a frontier coordinator plans and synthesizes while cheap workers do the token-hungry web reading in their own threads (84-98% of input tokens at the cheap rate; 2.5x cheaper and 3x faster than a rigor-matched solo frontier agent on the authors' runs). Covers per-thread cost attribution via each session thread's typed cumulative `usage` and pricing a run against an all-frontier baseline. |
